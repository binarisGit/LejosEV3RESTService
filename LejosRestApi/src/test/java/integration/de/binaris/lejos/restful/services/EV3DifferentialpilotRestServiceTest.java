...
public class EV3DifferentialpilotRestServiceTest {
	...
	public void ev3mindstorm_should_run_5cm() {
		...
		assertThat("ev3mindstorm did not run 5 cm", response.getStatus(), is(equalTo(204)));
	}
	...
	public void ev3mindstorm_should_stop() {
		...
		assertThat("ev3mindstorm did not quickstopped!", response.getStatus(), is(equalTo(204)));
	}
	...
	public void ev3mindstorm_should_quickstop() {
		...
		assertThat("ev3mindstorm did not stopped!", response.getStatus(), is(equalTo(204)));
	}
	...
	public void ev3mindstorm_should_rotate() {
		...
		assertThat("ev3mindstorm did not rotated!", response.getStatus(), is(equalTo(204)));
	}

	@Test
	public void ev3mindstorm_should_get_movementincrement() {
		client.target(Config.BASE_URL + "/differentialpilot/run/5").request().get(Response.class);
		Float movementIncrement= client.target(Config.BASE_URL + "/differentialpilot/getmovementincrement").request().get(Float.class);
		assertThat("movementIncrement do not match", Math.round(movementIncrement), is(equalTo(5)));
	}
}
